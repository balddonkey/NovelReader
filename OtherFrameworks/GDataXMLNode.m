...
- (NSDictionary *)allProps {
    if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
        xmlAttr *attr = xmlNode_->properties;
        NSMutableDictionary *dict = [NSMutableDictionary new];
        while (attr != NULL) {
            NSString *name = [NSString stringWithCString:(const char *)attr->name encoding:NSUTF8StringEncoding];
            NSString *value = [NSString stringWithCString:(const char *)attr->children->content encoding:NSUTF8StringEncoding];
            dict[name] = value;
            attr = attr->next;
        }
        return dict;
...
- (NSString *)propForName:(NSString *)name {
    xmlChar *prop = xmlGetProp(xmlNode_, (xmlChar *)name.UTF8String);
    if (prop == NULL) {
        return nil;
    }
    return [NSString stringWithCString:(const char *)prop encoding:NSUTF8StringEncoding];
}

- (xmlAttrPtr)setProp:(NSString *)value forName:(NSString *)name {
    xmlAttrPtr attr = xmlNewProp(xmlNode_, (xmlChar *)name.UTF8String, (xmlChar *)value.UTF8String);
    return attr;
}

- (BOOL)unsetProp:(NSString *)name {
    BOOL result = xmlUnsetProp(xmlNode_, (xmlChar *)name.UTF8String);
    return result;
}
